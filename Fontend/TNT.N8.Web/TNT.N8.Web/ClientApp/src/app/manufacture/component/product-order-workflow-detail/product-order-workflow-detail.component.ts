...
class ConfigStageModel {
  ...
  employeeList: any;
  optionPersonInCharge: any;
  optionPersonVerifier: any;
}
...
export class ProductOrderWorkflowDetailComponent implements OnInit {
  ...
  initTable() {
    ...
    this.colsStage = [
      ...
      { field: 'stageGroupName', header: 'Nhóm công đoạn', width: '20%', textAlign: 'left', type: 'string' },
      ...
      //{ field: 'setting', header: 'Thao tác', width: '25%', textAlign: 'center', type: 'string' },
    ];
    ...
    this.colsSetting = [
      ...
      { field: 'sort', header: 'Thứ tự trong dòng', width: '6%', textAlign: 'left', type: 'string' },
      { field: 'line', header: 'Dòng', width: '6%', textAlign: 'center', type: 'string' },
      ...
      { field: 'valueDvt', header: 'Giá trị quy đổi theo ĐVT', width: '25%', textAlign: 'center', type: 'string' },
  ...
  async getMasterData() {
    this.loading = true;
    let [productResponse, organizationResponse, categoryResponse]: any = await Promise.all([
  ...
  getDetailById() {
    this.manufactureService.getConfigProductionById(this.productOrderWorkflowId).subscribe(response => {
      let result: any = response;
      this.loading = false;

      if (result.statusCode == 200) {
        this.configProductionData = result.configProductionModel;

        if (this.configProductionData != null) {
          var productById = this.productList.find(p => p.productId == this.configProductionData.productId);
          this.productCode = this.configProductionData.productCode;
          this.configCode = this.configProductionData.code;
          this.active = this.configProductionData.availability;
          this.nameControl.setValue(productById);
          this.descriptionControl.setValue(this.configProductionData.description);
          this.ltvControl.setValue(this.configProductionData.ltv);
          this.pcControl.setValue(this.configProductionData.pc);
          this.productNumberControl.setValue(this.configProductionData.productionNumber);
          this.isDefaultControl.setValue(this.configProductionData.availability);

          if (this.configProductionData.configStages != null || this.configProductionData.configStages != undefined) {
            this.configProductionData.configStages.forEach(async (item, index) => {
              item.stt = index + 1;

              let [employeeRespons]: any = await Promise.all([
                this.employeeService.searchEmployeeAsync('', '', '', '', [], item.departmentId)
              ]);
              item.employeeList = employeeRespons.employeeList;
              item.optionPersonVerifier = item.employeeList.find(e => e.employeeId == item.personVerifierId);
              item.optionPersonInCharge = [];
              if (item.personInChargeId != null && item.personInChargeId != undefined) {
                item.personInChargeId.forEach(ps => {
                  item.optionPersonInCharge.push(item.employeeList.find(e => e.employeeId == ps));
                });
              }
            });
          }
          if (this.stageGroupErrorList != null && this.stageGroupErrorList != undefined && this.configProductionData.inspectionStageId != null && this.configProductionData.inspectionStageId != undefined) {
            this.stageGroupErrorList.forEach(item => {
              item.checked = false;
              var isHasValues = this.configProductionData.inspectionStageId.find(c => c == item.categoryId);
              if (isHasValues != null && isHasValues != undefined) {
                item.checked = true;
              }
            });
          }
  ...
  updateProductOrderWorkflow(mode) {
    if (!this.productOrderWorkflowForm.valid) {
      ...
      //this.configProductionData.availability = this.isDefaultControl.value == null ? false : this.isDefaultControl.value;

      this.loading = true;
      this.awaitResult = true;
      console.log(this.configProductionData);
  ...
  resetForm() {
    ...
    this.productNumberControl.setValue(null);
  ...
  async showCreateStageDialog(event) {
    ...
    var dateNowDDMMYYYY = new Date(dateNow.getFullYear(), dateNow.getMonth(), dateNow.getDate())
    var detail = productDetailResponse.listConfigurationProduct.find(c => new Date(c.startDate) <= dateNowDDMMYYYY && new Date(c.endDate) >= dateNowDDMMYYYY)
    
    this.stageConfigNVL = [];
    if (detail != null && detail != undefined) {
      if (detail.listConfigurationProductMapping != null && detail.listConfigurationProductMapping != undefined) {
        detail.listConfigurationProductMapping.forEach(item => {
          var nvlByProduct = nvlRespons.productList.find(c => c.productId == item.productId);
          console.log(nvlByProduct)
          if (nvlByProduct != null && nvlByProduct != undefined && nvlByProduct.accountingCode != null && nvlByProduct.accountingCode != undefined) {
            var nvlByAccount = nvlRespons.productList.filter(c => c.accountingCode == nvlByProduct.accountingCode);
            if (nvlByAccount != null && nvlByAccount != undefined) {
              this.stageConfigNVL = this.stageConfigNVL.concat(nvlByAccount);
            }
          }
        });
      }
    ...
    if (event == null) {
      ...
      this.productNVLList = [{ productId: null, productName: null, productCode: null, productUnitName: null }];
      ...
      let [employeeRespons]: any = await Promise.all([
        this.employeeService.searchEmployeeAsync('', '', '', '', [], this.selectedOrg.organizationId)
      ]);
      ...
      event.personInChargeId.forEach(item => {
        var emp = this.employeeByOrgList.find(c => c.employeeId == item);
      ...
      this.productNVLList = lstNewProduct.concat(this.stageConfigNVLChoose);
      ...
      if (event.configSpecificationsStages != null && event.configSpecificationsStages != undefined && event.configSpecificationsStages.length != 0) {
        ...
        event.configSpecificationsStages.forEach(item => {
          ...
          var step = this.stepProgressQC.find(c => (c.id != null && c.id != undefined && c.id != 0 && c.id == item.configStepByStepStageIdMapping) || ((c.id == null || c.id == undefined || c.id == 0) && c.stt == item.configStepByStepStageIdMapping));
          var content = this.stepProgressDetailQC.find(c => (c.id != null && c.id != undefined && c.id != 0 && c.id == item.configContentStageIdMapping) || ((c.id == null || c.id == undefined || c.id == 0) && c.stt == item.configContentStageIdMapping));
  ...
  saveCreateStage() {
    ...
    if (!this.stepStageForm.valid || this.validateStage()) {
      ...
      if (this.choosedRowStage != 0) {
        this.configProductionData.configStages.forEach(config => {
          if (config.stt == this.choosedRowStage) {
            ...
            config.optionPersonInCharge = this.selectedListEmployee.value;

            config.optionPersonVerifier = this.selectedVerifier.value;
            ...
            config.employeeList = this.employeeByOrgList;
      ...
      this.updateProductOrderWorkflow(null);
    }
  }
  ...
  onChangePerson(rowData) {
    var index = this.configProductionData.configStages.indexOf(rowData);
    if (index > -1) {
      this.configProductionData.configStages[index].personVerifierId = this.configProductionData.configStages[index].optionPersonVerifier.employeeId;
      this.configProductionData.configStages[index].personVerifierName = this.configProductionData.configStages[index].optionPersonVerifier.employeeName;
      this.configProductionData.configStages[index].personInChargeId = [];
      this.configProductionData.configStages[index].personInChargeName = [];
      this.configProductionData.configStages[index].optionPersonInCharge.forEach(item => {
        this.configProductionData.configStages[index].personInChargeId.push(item.employeeId);
        this.configProductionData.configStages[index].personInChargeName.push(item.employeeName);
      });

      this.manufactureService.saveConfigStageForPerson(this.configProductionData.configStages[index].id, this.configProductionData.configStages[index].personInChargeId, this.configProductionData.configStages[index].personVerifierId).subscribe(response => {
        let result: any = response;
        this.loading = false;
