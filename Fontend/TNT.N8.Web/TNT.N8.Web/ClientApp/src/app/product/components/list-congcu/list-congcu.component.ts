...
@Component({
  selector: "app-list-congcu",
  templateUrl: "./list-congcu.component.html",
  styleUrls: ["./list-congcu.component.css"],
})
export class ListCongCuComponent implements OnInit {
  first: number = 0;
  ...
  async getMasterdata() {
    ...
    let [masterDataResult, searchResult]: any = await Promise.all([
      this.productService.getListProduct(2),
      this.productService.searchProductAsync(2, productName, productCode),
    ]);
    this.loading = false;
    if (
  ...
  goToCreate() {
    this.router.navigate(["/product/create", { type: 2 }]);
  }

  goToDetail(productId: string) {
    this.router.navigate([
      "/product/detail",
      { productId: productId, type: 2 },
    ]);
  }
  ...
  async importExcel() {
    ...
    reader.onload = (e: any) => {
      ...
      listImport.shift();
      listImport = listImport.filter((e) => e[0] && e[3]);
      ...
      if (listImport && listImport.length > 0) {
        listImport.length &&
          listImport.forEach((_rawData) => {
            ...
            product.productUnitId = this.listUnitCategory.find(
              (c) =>
                c.categoryName.trim().toLowerCase() ===
                product.productUnitName?.trim().toLowerCase()
            )?.categoryId;
  ...
  openDetailImportDialog(listProductImport: Array<any>) {
    let ref = this.dialogService.open(ImportExcelNvlComponent, {
      data: {
        listProductCCDCImport: listProductImport,
      },
