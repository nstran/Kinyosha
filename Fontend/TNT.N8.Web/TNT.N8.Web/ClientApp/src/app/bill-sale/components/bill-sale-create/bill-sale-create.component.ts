    ...
    this.billSaleService.getMasterDataBillSaleCreateEdit(true, this.orderId).subscribe(response => {
      ...
      if (result.statusCode == 200) {
        ...
        if (this.orderId != null) {
          ...
          this.discountValue = orderTemp.discountValue.toString();
          if (orderTemp.discountType) {
            this.discountType = this.discountTypeList.find(x => x.code == 'PT');
          }
          else {
            this.discountType = this.discountTypeList.find(x => x.code == 'ST');
          }
        }
