...
export class DanhSachDeNghiTamUngComponent implements OnInit {
  ...
  delItem(rowData) {
    this.confirmationService.confirm({
      message: 'Bạn chắc chắn muốn xóa đề nghị tạm ứng?',
      accept: () => {
        this.quanLyCongtacService.xoaDeNghiTamHoanUng(rowData.deNghiTamHoanUngId).subscribe(response => {
          ...
          if (result.statusCode == 200) {
            ...
            let mgs = { severity: 'success', summary: 'Thông báo:', detail: "Xóa đề nghị tạm ứng thành công." };
            this.showMessage(mgs);
          } else {
  ...
  async searchDeNghiTamUng() {
    ...
    if (result.statusCode === 200) {
      ...
      if (this.listDeNghiTamHoanUng.length == 0) {
        let mgs = { severity: 'warn', summary: 'Thông báo', detail: 'Không tìm thấy đề nghị tạm ứng nào!' };
        this.showMessage(mgs);
      }
    }
  }
