    ...
    public class CustomerDAO : BaseDAO, ICustomerDataAccess
    {
        ...
        public GetCustomerByIdResult GetCustomerById(GetCustomerByIdParameter parameter)
        {
            try
            {
                ...
                using (var cn = new NpgsqlConnection(this.Configuration.GetConnectionString("DefaultConnection")))
                {
                    cn.Open();
                    NpgsqlCommand cmd = new NpgsqlCommand("call tnm_gettotalsaleandtotaltotalreceivablebycustomerid(:customerid, :totalsalevalue, :totalreceivable )", cn);
                    cmd.Parameters.AddWithValue("@customerid", NpgsqlDbType.Uuid, cusEntityModel.CustomerId);
                    cmd.Parameters.AddWithValue("@totalsalevalue", NpgsqlDbType.Integer, 0);
                    cmd.Parameters.AddWithValue("@totalreceivable", NpgsqlDbType.Integer, 0);
                    var dataReader = cmd.ExecuteReader();

                    if (dataReader != null)
                    {
                        cusEntityModel.TotalSaleValue = dataReader.GetOrdinal("totalsalevalue");
                        cusEntityModel.TotalReceivable = dataReader.GetOrdinal("totalreceivable");
                    }
                    cn.Close();
                }
